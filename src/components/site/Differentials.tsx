...
import { HeartHandshake, Sparkles, ShieldCheck, GraduationCap, Feather, Clock } from 'lucide-react'
import { DIFFERENTIALS } from '@/lib/site'
import { Reveal, SectionLabel } from './primitives'

const ICONS = [HeartHandshake, Sparkles, ShieldCheck, GraduationCap, Feather, Clock]
    ...
    <section className="relative bg-secondary/60 py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-5 md:px-10">
        <div className="max-w-2xl">
          <Reveal><SectionLabel>Por que o estúdio</SectionLabel></Reveal>
          <Reveal delay={0.1}>
            ...
            return (
              <Reveal key={d.title} delay={i * 0.08}>
                <div className="group relative h-full overflow-hidden rounded-2xl border border-border bg-card p-7 transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_30px_60px_-30px_rgba(46,37,32,0.4)]">
                  ...
                  <h3 className="relative mt-5 font-display text-xl text-espresso">{d.title}</h3>
                  <p className="relative mt-2 leading-relaxed text-espresso/60">{d.text}</p>
                </div>
              </Reveal>
            )
          })}
