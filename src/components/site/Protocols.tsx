    ...
    <section className="relative overflow-hidden py-24 md:py-32">
      ...
      <div className="mx-auto max-w-6xl px-5 md:px-10">
        <div className="mx-auto max-w-2xl text-center">
          <Reveal><SectionLabel center>Protocolos</SectionLabel></Reveal>
          <Reveal delay={0.1}>
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 md:grid-cols-2">
          <Reveal>
            <PolicyCard icon={<Clock className="size-5" />} title="Atendimento" items={ATENDIMENTO} />
          </Reveal>
          <Reveal delay={0.1}>
            <PolicyCard icon={<CalendarCheck className="size-5" />} title="Reserva & confirmação" items={RESERVA} />
          </Reveal>
        </div>

        <Reveal delay={0.12}>
          <div className="glass-dark relative mt-6 overflow-hidden rounded-[1.6rem] p-8 text-primary-foreground md:p-12">
            <div className="glow-rose pointer-events-none absolute -right-10 -top-10 size-48 rounded-full opacity-40 blur-3xl" aria-hidden />
            <div className="relative flex flex-col gap-6 md:flex-row md:items-center">
              <div className="grid size-14 shrink-0 place-items-center rounded-2xl bg-white/10 text-champagne">
                <ShieldAlert className="size-7" />
              </div>
              <div>
                <p className="tracking-luxe text-[10px] uppercase text-champagne">Informação importante</p>
  ...
  return (
    <div className="h-full rounded-[1.6rem] border border-border bg-card p-8">
      <div className="flex items-center gap-3">
        <span className="grid size-11 place-items-center rounded-xl bg-secondary text-gold">{icon}</span>
