        ...
        <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
          ...
          <Reveal delay={0.18}>
            <p className="max-w-xs text-espresso/60">
              Cada serviço é pensado para realçar a sua naturalidade com conforto
              e durabilidade.
            </p>
          </Reveal>
        </div>
        ...
        <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {PROCEDURES.map((p, i) => (
            <Reveal key={p.id} delay={(i % 4) * 0.08}>
              <motion.a
                href={waLink(`Olá! Tenho interesse no procedimento "${p.name}". Gostaria de agendar.`)}
                target="_blank"
                rel="noreferrer"
                whileHover={{ y: -8 }}
              ...
              >
                <div className="relative aspect-[4/5] overflow-hidden bg-champagne-soft">
                  ...
                  <span className="absolute right-3 top-3 grid size-9 place-items-center rounded-full bg-white/90 text-espresso opacity-0 transition-all duration-500 group-hover:opacity-100">
                ...
                <div className="flex flex-1 flex-col p-5">
                  ...
                  <p className="mt-1.5 flex-1 text-sm leading-relaxed text-espresso/60">
                    {p.detail}
                  </p>
                  {p.maintenance && (
                  ...
                  <div className="mt-4 flex items-end justify-between border-t border-border pt-4">
                    <span className="font-display text-[1.5rem] leading-none text-espresso">
                      {p.price}
                    </span>
