    ...
    <>
      <motion.header
        ...
        className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${
          scrolled
            ? "glass shadow-[0_8px_30px_-18px_rgba(46,37,32,0.3)]"
            : "bg-transparent"
        }`}
      >
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-10">
          <a href="#inicio" className="flex flex-col leading-none">
            <span className="font-display text-[1.35rem] tracking-tight text-espresso">
              Josiane Vieira
            </span>
            <span className="tracking-luxe text-[8px] uppercase text-gold">
              Beauty Studio
            </span>
          </a>
          ...
          <ul className="hidden items-center gap-7 lg:flex">
            {NAV_LINKS.map((l) => (
              <li key={l.href}>
                <a
                  href={l.href}
                  className="group relative text-sm text-espresso/70 transition-colors hover:text-espresso"
                >
                  {l.label}
          ...
          <div className="flex items-center gap-3">
            <a
              href={waLink("Olá! Gostaria de agendar um horário no Josiane Vieira Beauty Studio.")}
              target="_blank"
              rel="noreferrer"
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div
              aria-hidden
              className="glow-champagne pointer-events-none absolute -right-20 top-10 size-72 rounded-full blur-3xl"
            />
            <div className="flex items-center justify-between px-5 py-4 md:px-10">
              <span className="font-display text-[1.35rem] text-espresso">
                Josiane Vieira
              </span>
              <button
            ...
            <nav className="relative mt-6 flex flex-col px-6 md:px-10">
              {NAV_LINKS.map((l, i) => (
                ...
                >
                  ...
                  <span className="tracking-luxe text-[10px] text-gold/60">
                    0{i + 1}
                  </span>
                </motion.a>
              ))}
            ...
            >
              ...
              <p className="mt-5 text-center text-sm text-muted-foreground">
                {BRAND.instagramHandle} · {BRAND.whatsappLabel}
              </p>
            </motion.div>
          </motion.div>
        )}
